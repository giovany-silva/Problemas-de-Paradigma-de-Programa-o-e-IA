...
:- op(400,yfx,'#').    /* Notação do nó construtor */
...
%%%
%%%
%%%     Forma do estado que temos    A/B/C/D/E/F/G/H/I
%%%            Onde {A,...,I} = {0,...,8}
...
:- dynamic location/3.   %%% Então a localização de um
                         %%% bloco pode ser afirmada.
                         %%% Local declarado e retirado
                         %%%  pela animação de puzzle
...
play_back([]) :- cursor(1,24).  %%% Coloca o cursor fora
                                %%% do caminho
...
reverse_video :- put(27), put("["), put("7"), put("m").
   ...
   %%% Bloco de objetos
   %%% Cada bloco deve ser desenhado usando mapa de
   %%% caracteres
   %%% desenhado em local que é afirmado e recolhido
   %%%  porpalyback
character_map(N, [ [' ',' ',' ',' ',' ',' ',' '],
                   [' ',' ',' ', N ,' ',' ',' '],
                   [' ',' ',' ',' ',' ',' ',' '] ]).
   ...
   %%% move o bloco vazio (spot) para esquerda
